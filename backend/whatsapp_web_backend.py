...
class WhatsAppWeb(WebSocket):
    ...
    def handleMessage(self):
        try:
            ...
            if (
                "from" not in obj
                or obj["from"] != "api2backend"
                or "type" not in obj
                or not (
                    ("command" in obj and obj["command"] == "backend-connectWhatsApp")
            ...
            if obj["type"] == "call":
                ...
                if obj["command"] == "backend-connectWhatsApp":
                    ...
                    onOpenCallback = {
                        "func": lambda cbSelf: self.sendJSON(
                            mergeDicts(
                                {"type": "resource_connected", "resource": "whatsapp"},
                                getAttr(cbSelf, "args"),
                            ),
                            getAttr(cbSelf, "tag"),
                        ),
                    ...
                    onMessageCallback = {
                        "func": lambda obj, cbSelf, moreArgs=None: self.sendJSON(
                            mergeDicts(
                                mergeDicts(
                                    {
